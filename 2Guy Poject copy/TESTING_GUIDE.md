...
Backend starts on `http://127.0.0.1:43210` with an empty database ready for testing.
...
- **Action**:
  ...
  3. Enter Assignment ID and Points
...
✅ **No hardcoded data** — Start with blank database
✅ **All 11 use cases** — Can be tested with manual data entry
✅ **Real-time updates** — SSE verified for all event types
✅ **Clean UI** — No pre-populated form fields
✅ **Complete separation** — Backend on 43210, Frontend on 5173
✅ **Architecture** — Matches design.puml (verified in DESIGN_VS_IMPLEMENTATION.md)

---

## Troubleshooting

**"Cannot GET /api/events/..." error**

- Make sure backend is running: `npm run serve`
- Check port 43210 is available: `lsof -i :43210`

**Database already exists error**

- Run `npm run db:reset` to start fresh
- Or delete `data/app.db` manually: `rm data/app.db`

**Port 5173 already in use**

- Vite will automatically try 5174, 5175, etc.
- Or kill existing process: `lsof -i :5173 | tail -1 | awk '{print $2}' | xargs kill -9`

**Login fails**

- Any username/password works (authentication is simplified)
- Try: `student` / `pw` or `teacher` / `pw`
