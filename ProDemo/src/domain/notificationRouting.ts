...
export class StudentParentRoutingStrategy
  implements NotificationRoutingStrategy
{
  ...
  async findParentsByStudentId(studentId: string): Promise<string[]> {
    const parents = await this.parentStudentLinks.findByStudentId(studentId);
    return parents ? parents.map((link) => link.parentId) : [];
  }
}
