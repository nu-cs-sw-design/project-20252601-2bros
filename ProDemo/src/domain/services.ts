...
// Domain event bus; optionally logs events via AuditLogRepository
// (matches DomainEventBus ..> AuditLogRepository in the design).
export class DomainEventBus {
  private handlers: Record<string, Array<(event: KnownDomainEvent) => void>> =
    {};

  constructor(private auditLog?: AuditLogRepository) {}
  ...
  publish(event: KnownDomainEvent) {
    ...
    if (this.auditLog) {
      // Auditing is additive-only; it does not affect behavior
...
export class AuthService {
  ...
  // Overload to align with UML `getCurrentUser()`
