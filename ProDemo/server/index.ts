...
async function main() {
  ...
  app.post("/api/grades", async (req, res) => {
    ...
    try {
      // Ensure an assignment exists for this section so
      // grade entries can be associated with the correct class.
      const existingAssignment = await assignments.findById(assignmentId);
    ...
    try {
      ...
      for (const d of dashboards as any[]) {
        ...
        if (Array.isArray(d.feedbackSummary)) {
         ...
         if (Array.isArray(d.healthSummary)) {
          allHealth.push(...d.healthSummary);
        }
      }
      res.json({
